    ...
    (function () {
        ...
        // ========== ISOLATED SOUNDS ==========
        const mcqCorrectSound = new Audio("https://cdn.jsdelivr.net/gh/oneprt/sounds@main/correct.mp3");
        const mcqIncorrectSound = new Audio("https://cdn.jsdelivr.net/gh/oneprt/sounds@main/incorrect.mp3");

        function playMcqCorrect() {
            mcqCorrectSound.currentTime = 0;
            mcqCorrectSound.play().catch(() => { });
        }
        function playMcqIncorrect() {
            mcqIncorrectSound.currentTime = 0;
            mcqIncorrectSound.play().catch(() => { });
        }

        // ========== CONFETTI (safe shared canvas) ==========
        ...
        // ========== MAIN MCQ LOGIC ==========
        function initStaticMcqs() {
            document.querySelectorAll("section.mcqs details.mcq").forEach(mcq => {
                const options = Array.from(mcq.querySelectorAll("ol.mcq-options > li"));
                const answerEl = mcq.querySelector(".answer");
                const correctOptions = options.filter(li => li.classList.contains("c"));
                ...
                options.forEach(li => {
                    li.addEventListener("click", function () {
                        ...
                        const isCorrect = this.classList.contains("c");

                        if (isCorrect) {
                            this.classList.add("selected", "correct-selected");
                            playMcqCorrect();
                            mcqConfetti(70);
                            selectedCorrect++;

                            // reveal only when ALL correct options are clicked
                            if (selectedCorrect === correctOptions.length) {
                                reveal();
                            }
                        } else {
                            this.classList.add("selected", "incorrect-selected");
                            playMcqIncorrect();
                            reveal(); // always reveal on incorrect
                        }
                    });
                });
                ...
                function reveal() {
                    if (revealed) return;
                    if (!hasRealContent(answerEl)) return; // empty / only whitespace → never show
        ...
        if (document.readyState === "loading") {
            document.addEventListener("DOMContentLoaded", initStaticMcqs);
        } else {
            initStaticMcqs();
        }
    })();
